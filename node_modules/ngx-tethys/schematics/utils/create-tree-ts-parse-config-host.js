...
class TreeTsParseConfigHost {
    ...
    _readDirectory(fileDir, extensions, excludes, includes, depth) {
        ...
        return dirEntry.subfiles
            ...
            .filter(item => (excludes.length ? !excludes.some(exclude => (0, minimatch_1.default)(path.join(fileDir, item), exclude)) : true))
            .filter(item => (includes.length ? includes.some(include => (0, minimatch_1.default)(path.join(fileDir, item), include)) : false))
